...
std::string replace_substring(std::string string, const std::string string_to_replace, const std::string string_replace) {
    for (size_t position = 0; ; position += string_replace.length()) {

        position = string.find(string_to_replace, position);

        if (position == std::string::npos || string.empty()) break;

        string.erase(position, string_to_replace.length());
        string.insert(position, string_replace);
    }
    return string;
}
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // location of 'OR' would be potential injection
    if (sql.find("or") != std::string::npos) {
        std::string cleaned = sql.c_str();
        cleaned = replace_substring(cleaned, "\'", "\"");
        std::cout << "Detected 'OR'! Potential SQL Injection!" << std::endl;
        std::cout << "Stopped Query: " << sql.c_str() << std::endl;
        std::cout << "Query cleaned: " << cleaned << std::endl;
        return false;
    }
